...
public class Payment implements Task {
    ...
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(
                ...
                Click.on(ClientPageElements.COUNTRY_ELEMENT),
                Click.on(PaymentPageElements.PAYMENT_BUTTON),
                Click.on(PaymentPageElements.CONFIRM_BUTTON)

        );
    }
    public static  Payment type(String phone, String name, String amount){
        return  instrumented(Payment.class,phone,name,amount);
    }
    public static Payment select(String client){
        return instrumented(Payment.class,client);
    }
